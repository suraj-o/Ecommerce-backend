...
export const newCoupon=TryCatch(async(req,res,next)=>{
    ...
    if(!coupon || !amount)return next(new ErrorHandler("Please all feilds",400));
...
export const create=TryCatch(async(req,res,next)=>{
    ...
    if(!amount)return next(new ErrorHandler("Please add amount",400));
...
export const applyDiscount=TryCatch(async(req,res,next)=>{
    ...
    if(!discount )return next(new ErrorHandler("Invalid ID",404));
...
export const getAllCoupon=TryCatch(async(req,res,next)=>{
    ...
    if(!coupon )return next(new ErrorHandler("no coupons yet",404));
