...
export const addminDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-dasboard-stats")) {
        stats = JSON.parse(myCache.get("admin-dasboard-stats"));
    }
    // in case of data does'nt exists in cache memory then getting data from database    
        ...
        const [lastMonthProducts, thisMonthProducts, lastMonthOrders, thisMonthOrders, lastMonthUsers, thisMonthUsers, productCount, userCount, orderCount, lastSixMonthData, categories, femaleUserCount, latestOrders] = await Promise.all([
...
export const pieChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-dasboard-pie")) {
        charts = JSON.parse(myCache.get("admin-dasboard-pie"));
    }
    // in case of data does'nt exists in cache memory then getting data from database    
    else {
        const [processCount, sippedCount, deliverdCount, productCount, categories, productOutStock, roleCount, userDOB] = await Promise.all([
...
export const BarChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-bar-chart"))
        charts = JSON.parse(myCache.get("admin-bar-chart"));
    else {
        const today = new Date();
        const lastSixMonthAgo = new Date();
        lastSixMonthAgo.setMonth(lastSixMonthAgo.getMonth() - 6);
        ...
        const [lastSixMonthUser, lastSixMonthProducts, twelveMonthOrder] = await Promise.all([
...
export const LineChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-line-chart"))
        charts = JSON.parse(myCache.get("admin-line-chart"));
    else {
        const today = new Date();
        const twelveMonthAgo = new Date();
        twelveMonthAgo.setMonth(twelveMonthAgo.getMonth() - 12);
        ...
        const [twelvexMonthUser, twelveMonthProducts, twelveeMonthOrder] = await Promise.all([
